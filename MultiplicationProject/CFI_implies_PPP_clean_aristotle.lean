...
This project request had uuid: 7bc28122-7fb9-4966-8ce9-2de6d2273cb6
...
- theorem CFI_PPD_implies_PPP' {M : Type*} [CommMonoid M]
    (h_reduced : Reduced M) (h_ppd : PP_D M) (h_cfi : CFI M) (h_atomic : Atomic M) : PP_P M
-/
...
theorem CFI_PPD_implies_PPP' {M : Type*} [CommMonoid M]
  ...
  -- Since $p$ is an atom, we know that $x$ and $y$ can only be $p^a$ for some $a$.
  have hx : ∀ {x : M}, x ∣ p^e → ∃ f : Finset ℕ, x = ∏ i ∈ f, p := by
    intro x hx
    obtain ⟨f, hf⟩ : ∃ f : Multiset M, x = f.prod ∧ ∀ a ∈ f, a ∈ Atoms M := by
      by_cases hx_unit : IsUnit x;
      · -- Since $x$ is a unit, by the definition of a reduced monoid, $x$ must be $1$.
        have hx_one : x = 1 := by
          exact h_reduced x hx_unit;
        exact ⟨ ∅, hx_one, by simp +decide ⟩;
      · exact Exists.imp ( by aesop ) ( h_atomic x hx_unit );
    -- Since $p$ is an atom, each element in $f$ must be $p$.
    have hf_p : ∀ a ∈ f, a = p := by
      intro a ha
      have h_div : a ∣ p^e := by
        exact dvd_trans ( Multiset.dvd_prod ha ) ( hf.1 ▸ hx );
      apply atom_dvd_power_eq_clean h_reduced h_cfi h_atomic hp (hf.2 a ha) h_div;
    use Finset.range (Multiset.card f);
    rw [ hf.1, Multiset.eq_replicate_of_mem hf_p ] ; simp +decide
  have hy : ∀ {y : M}, y ∣ p^e → ∃ g : Finset ℕ, y = ∏ i ∈ g, p := by
    exact fun { y } hy => hx hy;
  obtain ⟨ g, hg ⟩ := hx ( dvd_of_mul_right_eq _ he.symm ) ; obtain ⟨ f, hf ⟩ := hy ( dvd_of_mul_left_eq _ he.symm ) ; aesop;
