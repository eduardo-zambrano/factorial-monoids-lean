...
# Example 10.2: The TD Failure Monoid (Failure of TD only)
...
- `TDFailMonoid`: The monoid with p₁² = p₂²
- `tdfail_reduced`: The monoid is reduced
- `tdfail_atomic`: The monoid is atomic
- `tdfail_tower_faithful`: tower faithfulness holds (powers within each tower are distinct)
- `tdfail_not_TD`: TD fails (p₁² = p₂² with p₁ ≠ p₂)
- `tdfail_CFI`: CFI holds (coprime factorizations combine independently)
- `tdfail_CPL`: CPL holds (p₃, p₄, ... give arbitrarily long coprime tuples)
-/
...
/-!
## The TD Failure Monoid Structure
...
structure PreTDMonoid where
  exp_p1 : ℕ
  exp_p2 : ℕ
  others : ℕ →₀ ℕ
  deriving DecidableEq

namespace PreTDMonoid

/-- Extensionality. -/
@[ext]
lemma ext {x y : PreTDMonoid} (h1 : x.exp_p1 = y.exp_p1)
    (h2 : x.exp_p2 = y.exp_p2) (h3 : x.others = y.others) : x = y := by
  cases x; cases y; simp at h1 h2 h3; simp [h1, h2, h3]

/-- The identity element. -/
protected def one : PreTDMonoid := ⟨0, 0, 0⟩

/-- Multiplication: add exponents. -/
protected def mul (x y : PreTDMonoid) : PreTDMonoid :=
  ⟨x.exp_p1 + y.exp_p1, x.exp_p2 + y.exp_p2, x.others + y.others⟩

instance : One PreTDMonoid := ⟨PreTDMonoid.one⟩
instance : Mul PreTDMonoid := ⟨PreTDMonoid.mul⟩

@[simp] lemma one_exp_p1 : (1 : PreTDMonoid).exp_p1 = 0 := rfl
@[simp] lemma one_exp_p2 : (1 : PreTDMonoid).exp_p2 = 0 := rfl
@[simp] lemma one_others : (1 : PreTDMonoid).others = 0 := rfl

@[simp] lemma mul_exp_p1 (x y : PreTDMonoid) : (x * y).exp_p1 = x.exp_p1 + y.exp_p1 := rfl
@[simp] lemma mul_exp_p2 (x y : PreTDMonoid) : (x * y).exp_p2 = x.exp_p2 + y.exp_p2 := rfl
@[simp] lemma mul_others (x y : PreTDMonoid) : (x * y).others = x.others + y.others := rfl

instance : CommMonoid PreTDMonoid where
  mul := (· * ·)
...
/-- The atom p₁. -/
def p1 : PreTDMonoid := ⟨1, 0, 0⟩

/-- The atom p₂. -/
def p2 : PreTDMonoid := ⟨0, 1, 0⟩

/-- The atom p_{n+3}. -/
def pn (n : ℕ) : PreTDMonoid := ⟨0, 0, Finsupp.single n 1⟩
...
def tdEquiv : PreTDMonoid → PreTDMonoid → Prop :=
...
@[simp] lemma tdEquiv_def (x y : PreTDMonoid) : tdEquiv x y ↔
    (x.others = y.others ∧ x.exp_p1 + x.exp_p2 = y.exp_p1 + y.exp_p2 ∧ x.exp_p1 % 2 = y.exp_p1 % 2) :=
  Iff.rfl

/-- tdEquiv is an equivalence relation. -/
lemma tdEquiv_equiv : Equivalence tdEquiv := by
  constructor
  · intro x; simp [tdEquiv]
...
/-- tdEquiv is compatible with multiplication. -/
lemma tdEquiv_mul : ∀ x₁ x₂ y₁ y₂ : PreTDMonoid,
    tdEquiv x₁ y₁ → tdEquiv x₂ y₂ → tdEquiv (x₁ * x₂) (y₁ * y₂) := by
  intro x₁ x₂ y₁ y₂ ⟨h1a, h1b, h1c⟩ ⟨h2a, h2b, h2c⟩
  simp only [tdEquiv, PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2, PreTDMonoid.mul_others]
  constructor
...
instance tdSetoid : Setoid PreTDMonoid where
  r := tdEquiv
  iseqv := tdEquiv_equiv

/-- The TD Failure Monoid: quotient of PreTDMonoid by the relation p₁² = p₂².
    Using `abbrev` to ensure instance unification with `Quotient tdSetoid`. -/
abbrev TDFailMonoid := Quotient tdSetoid

namespace TDFailMonoid

/-- Injection from PreTDMonoid to TDFailMonoid. -/
def mk (x : PreTDMonoid) : TDFailMonoid := Quotient.mk tdSetoid x

/-- One in the quotient. -/
protected def one' : TDFailMonoid := mk 1

/-- Multiplication in the quotient. -/
protected def mul' (a b : TDFailMonoid) : TDFailMonoid :=
  Quotient.lift₂ (fun x y => mk (x * y))
    (fun _ _ _ _ hxy hzw => Quotient.sound (tdEquiv_mul _ _ _ _ hxy hzw)) a b

instance : One TDFailMonoid := ⟨TDFailMonoid.one'⟩
instance : Mul TDFailMonoid := ⟨TDFailMonoid.mul'⟩

@[simp] lemma mk_one : mk 1 = (1 : TDFailMonoid) := rfl

lemma mk_mul (x y : PreTDMonoid) : mk x * mk y = mk (x * y) := rfl

/-- Lemma relating Quotient.mk to our mk. -/
lemma quot_mk_eq_mk (x : PreTDMonoid) : Quotient.mk tdSetoid x = mk x := rfl

instance : CommMonoid TDFailMonoid where
  mul := (· * ·)
  mul_assoc := by
    intro a b c
    ...
    refine ⟨?_, ?_, ?_⟩
    · simp only [PreTDMonoid.mul_others]
      abel
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2]
      omega
    · simp only [PreTDMonoid.mul_exp_p1]
      omega
  one := 1
  one_mul := by
    intro a
    ...
    apply Quotient.sound
    refine ⟨?_, ?_, ?_⟩
    · simp only [PreTDMonoid.mul_others, PreTDMonoid.one_others, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                 PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.one_exp_p1, zero_add]
  mul_one := by
    intro a
    obtain ⟨a', rfl⟩ := Quotient.exists_rep a
    rw [quot_mk_eq_mk]
    ...
    apply Quotient.sound
    refine ⟨?_, ?_, ?_⟩
    · simp only [PreTDMonoid.mul_others, PreTDMonoid.one_others, add_zero]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                 PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2, add_zero]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.one_exp_p1, add_zero]
  mul_comm := by
    intro a b
    obtain ⟨a', rfl⟩ := Quotient.exists_rep a
    obtain ⟨b', rfl⟩ := Quotient.exists_rep b
...
def atomP1 : TDFailMonoid := mk PreTDMonoid.p1

/-- The atom p₂ in the quotient. -/
def atomP2 : TDFailMonoid := mk PreTDMonoid.p2

/-- The atom p_{n+3} in the quotient. -/
def atomPn (n : ℕ) : TDFailMonoid := mk (PreTDMonoid.pn n)
...
theorem p1_sq_eq_p2_sq : atomP1 ^ 2 = atomP2 ^ 2 := by
  ...
  apply Quotient.sound
  refine ⟨?_, ?_, ?_⟩
  · simp only [PreTDMonoid.mul_others, PreTDMonoid.p1_others, PreTDMonoid.p2_others, add_zero]
  · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
               PreTDMonoid.p1_exp_p1, PreTDMonoid.p1_exp_p2,
               PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2, add_zero, zero_add]
  · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p1_exp_p1, PreTDMonoid.p2_exp_p1, add_zero]

/-- p₁ ≠ p₂ in the quotient. -/
theorem p1_ne_p2 : atomP1 ≠ atomP2 := by
  intro h
  have heq : mk PreTDMonoid.p1 = mk PreTDMonoid.p2 := h
  have hequiv := Quotient.exact heq
  obtain ⟨_, _, hparity⟩ := hequiv
  simp only [PreTDMonoid.p1_exp_p1, PreTDMonoid.p2_exp_p1] at hparity
  norm_num at hparity
...
lemma isUnit_iff (x : TDFailMonoid) : IsUnit x ↔ x = 1 := by
  constructor
  · intro ⟨u, hu⟩
    ...
    simp only [PreTDMonoid.mul_others, PreTDMonoid.one_others] at hothers
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
               PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2] at htotal
...
/-- The TD Failure Monoid is reduced. -/
theorem tdfail_reduced : Reduced TDFailMonoid := by
  intro x hx
...
lemma atomP1_not_isUnit : ¬IsUnit atomP1 := by
  ...
  obtain ⟨_, _, hparity⟩ := hequiv
  simp only [PreTDMonoid.p1_exp_p1, PreTDMonoid.one_exp_p1] at hparity
  norm_num at hparity
...
lemma atomP2_not_isUnit : ¬IsUnit atomP2 := by
  ...
  simp only [PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2,
             PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2] at htotal
  omega
...
lemma atomPn_not_isUnit (m : ℕ) : ¬IsUnit (atomPn m) := by
  ...
  simp only [PreTDMonoid.pn_others, PreTDMonoid.one_others] at hothers
...
lemma atomP1_irreducible : Irreducible atomP1 := by
  ...
  · intro a b hab
    ...
    simp only [PreTDMonoid.mul_others, PreTDMonoid.p1_others] at hothers
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
               PreTDMonoid.p1_exp_p1, PreTDMonoid.p1_exp_p2] at htotal
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p1_exp_p1] at hparity
...
lemma atomP2_irreducible : Irreducible atomP2 := by
  ...
  · intro a b hab
    ...
    have hequiv := Quotient.exact hab
    obtain ⟨hothers, htotal, _⟩ := hequiv
    simp only [PreTDMonoid.mul_others, PreTDMonoid.p2_others] at hothers
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
               PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2] at htotal
    have ⟨ha_f, hb_f⟩ := finsupp_add_eq_zero_iff.mp hothers.symm
    have hsum : a'.exp_p1 + a'.exp_p2 + (b'.exp_p1 + b'.exp_p2) = 1 := by omega
    cases Nat.eq_zero_or_pos (a'.exp_p1 + a'.exp_p2) with
    | inl ha =>
...
lemma atomPn_irreducible (m : ℕ) : Irreducible (atomPn m) := by
  ...
  · intro a b hab
    ...
    have hequiv := Quotient.exact hab
    obtain ⟨hothers, htotal, _⟩ := hequiv
    simp only [PreTDMonoid.mul_others, PreTDMonoid.pn_others] at hothers
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
               PreTDMonoid.pn_exp_p1, PreTDMonoid.pn_exp_p2] at htotal
...
private def tdSize (x : PreTDMonoid) : ℕ :=
  x.exp_p1 + x.exp_p2 + x.others.sum (fun _ v => v)

/-- The TD Failure Monoid is atomic. -/
theorem tdfail_atomic : Atomic TDFailMonoid := by
  intro x
  obtain ⟨x', rfl⟩ := Quotient.exists_rep x
  have h : ∀ n (y : PreTDMonoid), tdSize y = n → ¬IsUnit (mk y) →
      ∃ L : List TDFailMonoid, (∀ a ∈ L, a ∈ Atoms TDFailMonoid) ∧ L.prod = mk y := by
    intro n
    induction n using Nat.strong_induction_on with
    | _ n ih =>
      intro y hsize hy_not_unit
      by_cases hy_atom : mk y ∈ Atoms TDFailMonoid
      ...
      · -- Factor out an atom
        ...
        · -- Factor out either p1 or p2
          ...
          · -- Factor out p1
            ...
            · have heq := (isUnit_iff (mk z)).mp hz_unit
              ...
              obtain ⟨hothers, htotal, _⟩ := hequiv
              exfalso
              simp only [PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2, hz_exp1, hz_exp2] at htotal
              ...
              have hy_is_p1 : mk y = atomP1 := by
                apply Quotient.sound
                refine ⟨?_, ?_, ?_⟩
                · simp only [PreTDMonoid.p1_others, hf0]
                · simp [hp1_val, hp2_val]
                · simp [hp1_val]
              rw [hy_is_p1] at hy_atom
              exact hy_atom atomP1_irreducible
            · have ⟨Lz, hLz_atoms, hLz_prod⟩ := ih (tdSize z) (hsize ▸ hz_size) z rfl hz_unit
              ...
              · simp only [List.prod_cons, hLz_prod, atomP1, mk_mul]
                apply Quotient.sound
                refine ⟨?_, ?_, ?_⟩
                · simp only [PreTDMonoid.mul_others, PreTDMonoid.p1_others, zero_add, hz_others]
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                             PreTDMonoid.p1_exp_p1, PreTDMonoid.p1_exp_p2, hz_exp1, hz_exp2]
                  omega
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p1_exp_p1, hz_exp1]
                  omega
          · -- y.exp_p1 = 0, so y.exp_p2 ≥ 1, factor out p2
            push_neg at hp1
            have hp1_0 : y.exp_p1 = 0 := by omega
            have hp2_pos : y.exp_p2 ≥ 1 := by omega
            let z : PreTDMonoid := ⟨y.exp_p1, y.exp_p2 - 1, y.others⟩
            ...
            · have heq := (isUnit_iff (mk z)).mp hz_unit
              ...
              have hy_is_p2 : mk y = atomP2 := by
                apply Quotient.sound
                refine ⟨?_, ?_, ?_⟩
                · simp only [PreTDMonoid.p2_others, hf0]
                · simp [hp1_0, hp2_val]
                · simp [hp1_0]
            ...
            · have ⟨Lz, hLz_atoms, hLz_prod⟩ := ih (tdSize z) (hsize ▸ hz_size) z rfl hz_unit
              ...
              · simp only [List.prod_cons, hLz_prod, atomP2, mk_mul]
                apply Quotient.sound
                refine ⟨?_, ?_, ?_⟩
                · simp only [PreTDMonoid.mul_others, PreTDMonoid.p2_others, zero_add, hz_others]
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                             PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2, hz_exp1, hz_exp2]
                  omega
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p2_exp_p1, zero_add, hz_exp1]
        · -- y.exp_p1 + y.exp_p2 = 0, so factor out some pn
          ...
          · have hsupp := Finsupp.support_nonempty_iff.mpr hf
            ...
            let z : PreTDMonoid := ⟨0, 0, y.others - Finsupp.single i 1⟩
            ...
            · have heq := (isUnit_iff (mk z)).mp hz_unit
              ...
              have hf1 : y.others = Finsupp.single i 1 := by
                ...
                simp only [PreTDMonoid.one_others, Finsupp.coe_zero, Pi.zero_apply] at hk
                rw [hz_others] at hk
              ...
              have hy_is_pn : mk y = atomPn i := by
                apply Quotient.sound
                refine ⟨?_, ?_, ?_⟩
                · simp only [PreTDMonoid.pn_others, hf1]
                · simp [hp1_0, hp2_0]
                · simp [hp1_0]
            ...
            · have ⟨Lz, hLz_atoms, hLz_prod⟩ := ih (tdSize z) (hsize ▸ hz_size) z rfl hz_unit
              ...
              · simp only [List.prod_cons, hLz_prod, atomPn, mk_mul]
                ...
                refine ⟨?_, ?_, ?_⟩
                · ext k
                  simp only [PreTDMonoid.mul_others, PreTDMonoid.pn_others,
                ...
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                             PreTDMonoid.pn_exp_p1, PreTDMonoid.pn_exp_p2, hz_exp1, hz_exp2]
                  omega
                · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.pn_exp_p1, zero_add, hz_exp1, hp1_0]
  intro hx_not_unit
  rw [quot_mk_eq_mk] at hx_not_unit
  obtain ⟨L, hL_atoms, hL_prod⟩ := h (tdSize x') x' rfl hx_not_unit
  use L
  constructor
...
lemma pow_atomP1 (n : ℕ) : atomP1 ^ n = mk ⟨n, 0, 0⟩ := by
  ...
  | succ m ih =>
    ...
    apply Quotient.sound
    refine ⟨?_, ?_, ?_⟩
    · simp only [PreTDMonoid.mul_others, PreTDMonoid.p1_others, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                 PreTDMonoid.p1_exp_p1, PreTDMonoid.p1_exp_p2, add_zero]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p1_exp_p1]
...
lemma pow_atomP2 (n : ℕ) : atomP2 ^ n = mk ⟨0, n, 0⟩ := by
  ...
  | succ m ih =>
    ...
    apply Quotient.sound
    refine ⟨?_, ?_, ?_⟩
    · simp only [PreTDMonoid.mul_others, PreTDMonoid.p2_others, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                 PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p2_exp_p1, zero_add]
...
lemma pow_atomPn (m k : ℕ) : atomPn m ^ k = mk ⟨0, 0, Finsupp.single m k⟩ := by
  ...
  | succ n ih =>
    ...
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                 PreTDMonoid.pn_exp_p1, PreTDMonoid.pn_exp_p2, zero_add]
    · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.pn_exp_p1, zero_add]

/-- The atoms of TDFailMonoid are {p₁, p₂} ∪ {pn(m) : m ∈ ℕ}.
...
lemma atoms_eq : Atoms TDFailMonoid = {atomP1, atomP2} ∪ { x | ∃ m, x = atomPn m } := by
  ext x
  ...
  · -- Forward direction: irreducible implies one of p1, p2, or pn m
    ...
    · -- Case: others = 0, so x' only has p1/p2 components
      ...
      · by_cases hp12_1 : x'.exp_p1 + x'.exp_p2 = 1
        ...
        · -- Total ≥ 2: x' factors non-trivially as p1 (or p2) × rest
          ...
          · -- Factor out p1
            let a : PreTDMonoid := ⟨1, 0, 0⟩
            let b : PreTDMonoid := ⟨x'.exp_p1 - 1, x'.exp_p2, 0⟩
            ...
            have hab : mk x' = mk a * mk b := by
              ...
              apply Quotient.sound
              refine ⟨?_, ?_, ?_⟩
              · simp only [PreTDMonoid.mul_others, ha_others, hb_others, add_zero, hf]
              · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                           ha_p1, hb_p1, ha_p2, hb_p2]; omega
              · simp only [PreTDMonoid.mul_exp_p1, ha_p1, hb_p1]; omega
            have ha_nu : ¬IsUnit (mk a) := by
              rw [isUnit_iff]; intro h
              have hequiv := Quotient.exact h
              obtain ⟨_, htot, _⟩ := hequiv
              simp only [PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2, ha_p1, ha_p2] at htot
              omega
            have hb_nu : ¬IsUnit (mk b) := by
              rw [isUnit_iff]; intro h
              have hequiv := Quotient.exact h
              obtain ⟨_, htot, _⟩ := hequiv
              simp only [PreTDMonoid.one_exp_p1, PreTDMonoid.one_exp_p2, hb_p1, hb_p2] at htot
              omega
          ...
          · -- exp_p1 = 0, so exp_p2 ≥ 2. Factor out p2
            ...
            let a : PreTDMonoid := ⟨0, 1, 0⟩
            let b : PreTDMonoid := ⟨0, x'.exp_p2 - 1, 0⟩
            have ha_others : a.others = 0 := rfl
            have hb_others : b.others = 0 := rfl
            ...
            have hab : mk x' = mk a * mk b := by
              ...
              · simp only [PreTDMonoid.mul_exp_p1, ha_p1, hb_p1, hp1_0]
            have ha_nu : ¬IsUnit (mk a) := by
              rw [isUnit_iff]; intro h
              have hequiv := Quotient.exact h
              obtain ⟨_, htot, _⟩ := hequiv
    ...
    · -- Case: others ≠ 0
      ...
      · -- Has both p1/p2 and others: factors as (p1/p2 part) × (others part)
        exfalso
        let a : PreTDMonoid := ⟨x'.exp_p1, x'.exp_p2, 0⟩
        let b : PreTDMonoid := ⟨0, 0, x'.others⟩
        have ha_others : a.others = 0 := rfl
        ...
        have hab : mk x' = mk a * mk b := by
          ...
          apply Quotient.sound
          refine ⟨?_, ?_, ?_⟩
          · simp only [PreTDMonoid.mul_others, ha_others, hb_others, zero_add]
          · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                       ha_p1, hb_p1, ha_p2, hb_p2, add_zero]
          · simp only [PreTDMonoid.mul_exp_p1, ha_p1, hb_p1, add_zero]
        have ha_nu : ¬IsUnit (mk a) := by
          rw [isUnit_iff]; intro h
          have hequiv := Quotient.exact h
          obtain ⟨_, htot, _⟩ := hequiv
        ...
        have hb_nu : ¬IsUnit (mk b) := by
          ...
          simp only [PreTDMonoid.one_others, hb_others] at hoth
          exact hf hoth
        rcases hx.isUnit_or_isUnit hab with ha | hb
        · exact ha_nu ha
        · exact hb_nu hb
      · -- exp_p1 = exp_p2 = 0, only others: check if it's a single atom
        ...
        · -- x' is atomPn i
          ...
          apply Quotient.sound
          refine ⟨hsingle, ?_, ?_⟩
          · simp only [PreTDMonoid.pn_exp_p1, PreTDMonoid.pn_exp_p2, hp1_0, hp2_0]
          · simp only [PreTDMonoid.pn_exp_p1, hp1_0]
        · -- others is not a single atom: factors non-trivially
          ...
          · -- Factor out one copy of pn i
            let a : PreTDMonoid := ⟨0, 0, Finsupp.single i 1⟩
            let b : PreTDMonoid := ⟨0, 0, x'.others - Finsupp.single i 1⟩
            have ha_p1 : a.exp_p1 = 0 := rfl
            have ha_p2 : a.exp_p2 = 0 := rfl
            ...
            have hab : mk x' = mk a * mk b := by
              ...
              apply Quotient.sound
              refine ⟨?_, ?_, ?_⟩
              · simp only [PreTDMonoid.mul_others, ha_others, hb_others]
                ext k
              ...
              · simp only [PreTDMonoid.mul_exp_p1, ha_p1, hb_p1, hp1_0, add_zero]
...
/-- tower faithfulness holds: For each atom p_i, powers p_i^a = p_i^b implies a = b. -/
theorem tdfail_tower_faithful : TowerFaithful TDFailMonoid := by
  intro p hp
...
/-- TD fails: p₁² = p₂² with p₁ ≠ p₂. -/
theorem tdfail_not_TD : ¬ TD TDFailMonoid := by
  intro htd
  -- p₁ and p₂ are atoms
  have hp1_atom : atomP1 ∈ Atoms TDFailMonoid := atomP1_irreducible
  have hp2_atom : atomP2 ∈ Atoms TDFailMonoid := atomP2_irreducible
  -- p₁² = p₂² with 2 ≥ 1
  have h := htd atomP1 atomP2 hp1_atom hp2_atom 2 2 (by omega) (by omega) p1_sq_eq_p2_sq
...
lemma atomP1_dvd_iff (x : TDFailMonoid) : atomP1 ∣ x ↔
    ∃ x' : PreTDMonoid, mk x' = x ∧ x'.exp_p1 ≥ 1 := by
  constructor
  · intro ⟨c, hc⟩
    obtain ⟨c', rfl⟩ := Quotient.exists_rep c
    -- The product p1 * c' is a representative of x with exp_p1 = 1 + c'.exp_p1 ≥ 1
    use PreTDMonoid.p1 * c'
    constructor
...
lemma atomP2_dvd_iff (x : TDFailMonoid) : atomP2 ∣ x ↔
    ∃ x' : PreTDMonoid, mk x' = x ∧ x'.exp_p2 ≥ 1 := by
  constructor
  · intro ⟨c, hc⟩
    obtain ⟨c', rfl⟩ := Quotient.exists_rep c
    -- The product p2 * c' is a representative of x with exp_p2 = 1 + c'.exp_p2 ≥ 1
    use PreTDMonoid.p2 * c'
    constructor
    · rw [quot_mk_eq_mk, atomP2, mk_mul] at hc
      exact hc.symm
    · simp only [PreTDMonoid.mul_exp_p2, PreTDMonoid.p2_exp_p2]
      omega
  · intro ⟨x', hx', hexp⟩
    rw [← hx']
...
lemma atomPn_dvd_iff (i : ℕ) (x : TDFailMonoid) : atomPn i ∣ x ↔
    ∃ x' : PreTDMonoid, mk x' = x ∧ x'.others i ≥ 1 := by
  constructor
  · intro ⟨c, hc⟩
    obtain ⟨x', rfl⟩ := Quotient.exists_rep x
    obtain ⟨c', rfl⟩ := Quotient.exists_rep c
...
lemma areCoprime_others (x y : TDFailMonoid) (hxy : AreCoprime x y) :
    ∀ x' y' : PreTDMonoid, mk x' = x → mk y' = y →
...
lemma not_dvd_of_dvd (p x a : TDFailMonoid) (hpx : ¬p ∣ x) (ha : a ∣ x) : ¬p ∣ a := by
  intro hpa
...
lemma exp_p1_lt_two_of_not_dvd_p2 (x : PreTDMonoid) (h_not_dvd : ¬atomP2 ∣ mk x)
...
lemma exp_p2_lt_two_of_not_dvd_p1 (x : PreTDMonoid) (h_not_dvd : ¬atomP1 ∣ mk x)
...
/-- Key cancellation lemma: In TDFailMonoid, a * b = a * c implies b = c.
    This holds because the monoid embeds into a free abelian structure. -/
lemma mul_left_cancel (a b c : TDFailMonoid) : a * b = a * c → b = c := by
  obtain ⟨a', rfl⟩ := Quotient.exists_rep a
  obtain ⟨b', rfl⟩ := Quotient.exists_rep b
  obtain ⟨c', rfl⟩ := Quotient.exists_rep c
  rw [quot_mk_eq_mk, quot_mk_eq_mk, quot_mk_eq_mk]
  ...
  have hequiv := Quotient.exact h
  obtain ⟨hothers, htotal, hparity⟩ := hequiv
  simp only [PreTDMonoid.mul_others, PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2] at hothers htotal hparity
  apply Quotient.sound
  refine ⟨?_, ?_, ?_⟩
...
/-- Right cancellation. -/
lemma mul_right_cancel (a b c : TDFailMonoid) : a * c = b * c → a = b := by
...
lemma coprime_product_unique (x y a₁ a₂ b₁ b₂ : TDFailMonoid) (_hxy : AreCoprime x y)
    (_hprod : a₁ * a₂ = b₁ * b₂) :
...
lemma dvd_others_support (a x : TDFailMonoid) (hdvd : a ∣ x)
    (a' x' : PreTDMonoid) (ha' : mk a' = a) (hx' : mk x' = x) :
  ...
  simp only [PreTDMonoid.mul_others] at hothers
...
lemma coprime_dvd_others_eq (x y a b a' b' : TDFailMonoid)
    (hxy : AreCoprime x y) (hprod : a * b = a' * b')
    (ha_dvd : a ∣ x) (hb_dvd : b ∣ y) (ha'_dvd : a' ∣ x) (hb'_dvd : b' ∣ y)
    (ar br ar' br' xr yr : PreTDMonoid)
  ...
  obtain ⟨hothers_eq, _, _⟩ := hequiv
  simp only [PreTDMonoid.mul_others] at hothers_eq
...
/-- For coprime x, y at the PreTDMonoid level, atoms are partitioned:
    For each index j, at most one of x.others j > 0 or y.others j > 0. -/
lemma coprime_others_disjoint (x y : TDFailMonoid) (hxy : AreCoprime x y)
    (x' y' : PreTDMonoid) (hx' : mk x' = x) (hy' : mk y' = y) :
...
lemma atom_dvd_coprime (p x y : TDFailMonoid) (hp : p ∈ Atoms TDFailMonoid)
  ...
  · -- p = atomP1
    ...
    obtain ⟨_, htotal, hparity⟩ := hequiv
    simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2] at htotal hparity
...
lemma coprime_split_exists (x y z : TDFailMonoid) (hxy : AreCoprime x y) (hz : z ∣ x * y) :
    ∃ a b, a ∣ x ∧ b ∣ y ∧ a * b = z := by
  obtain ⟨z', rfl⟩ := Quotient.exists_rep z
  rw [quot_mk_eq_mk]
  revert x y
  suffices h : ∀ n (z' : PreTDMonoid), tdSize z' = n →
      ∀ x y : TDFailMonoid, AreCoprime x y → mk z' ∣ x * y →
      ∃ a b, a ∣ x ∧ b ∣ y ∧ a * b = mk z' from
    fun x y => h (tdSize z') z' rfl x y
  intro n
  induction n using Nat.strong_induction_on with
  | _ n ih =>
    ...
    · -- Factor out an atom from z' based on its representative structure
      ...
      have factor_left (p : TDFailMonoid) (z₁ : PreTDMonoid) (_hp : p ∈ Atoms TDFailMonoid)
          (hfact : mk z' = p * mk z₁) (hsize₁ : tdSize z₁ < n) (hpx : p ∣ x) :
        ...
        obtain ⟨a₁, b, ha₁, hb, hab⟩ := ih (tdSize z₁) hsize₁ z₁ rfl x₁ y hxy₁ hz₁
        refine ⟨p * a₁, b, ?_, hb, ?_⟩
        · obtain ⟨c, hc⟩ := ha₁
          exact ⟨c, by rw [hx₁, hc, mul_assoc]⟩
        · rw [mul_assoc, hab]; exact hfact.symm
      -- Helper: symmetric case, atom divides y
      have factor_right (p : TDFailMonoid) (z₁ : PreTDMonoid) (_hp : p ∈ Atoms TDFailMonoid)
          (hfact : mk z' = p * mk z₁) (hsize₁ : tdSize z₁ < n) (hpy : p ∣ y) :
          ∃ a b, a ∣ x ∧ b ∣ y ∧ a * b = mk z' := by
        obtain ⟨y₁, hy₁⟩ := hpy
        have hz₁ : mk z₁ ∣ x * y₁ := by
          obtain ⟨w, hw⟩ := hz
      ...
      · -- Factor out atomP1
        let z₁ : PreTDMonoid := ⟨z'.exp_p1 - 1, z'.exp_p2, z'.others⟩
        have hfact : mk z' = atomP1 * mk z₁ := by
          rw [atomP1, mk_mul]; apply Quotient.sound
          refine ⟨?_, ?_, ?_⟩
          · simp only [PreTDMonoid.mul_others, PreTDMonoid.p1_others, zero_add, z₁]
          · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                PreTDMonoid.p1_exp_p1, PreTDMonoid.p1_exp_p2, z₁]; omega
          · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p1_exp_p1, z₁]; omega
        have hsize₁ : tdSize z₁ < n := by
          rw [← hsize]; unfold tdSize; simp [z₁]; omega
      ...
      · by_cases hp2 : z'.exp_p2 ≥ 1
        · -- Factor out atomP2
          let z₁ : PreTDMonoid := ⟨z'.exp_p1, z'.exp_p2 - 1, z'.others⟩
          have hfact : mk z' = atomP2 * mk z₁ := by
            rw [atomP2, mk_mul]; apply Quotient.sound
            refine ⟨?_, ?_, ?_⟩
            · simp only [PreTDMonoid.mul_others, PreTDMonoid.p2_others, zero_add, z₁]
            · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
                  PreTDMonoid.p2_exp_p1, PreTDMonoid.p2_exp_p2, z₁]; omega
            · simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.p2_exp_p1, z₁]; omega
          have hsize₁ : tdSize z₁ < n := by
            rw [← hsize]; unfold tdSize; simp [z₁]; omega
...
lemma labeledFact_one_unique (f : LabeledFactorizations 2 (1 : TDFailMonoid)) :
...
theorem tdfail_CFI : CFI TDFailMonoid := by
  intro x y hxy
  by_cases hx : x = 1
    ...
    · -- Case both x ≠ 1 and y ≠ 1: Use coprimality structure
      ...
      · -- Injectivity: show f = f' and g = g' from f(i) * g(i) = f'(i) * g'(i)
        ...
        have hf_eq : f = f' := by
          ...
          simp only [PreTDMonoid.mul_others, PreTDMonoid.mul_exp_p1,
                     PreTDMonoid.mul_exp_p2] at h_others_prod h_total_prod h_parity_prod
          ...
          · -- Total: fi.exp_p1 + fi.exp_p2 = fi'.exp_p1 + fi'.exp_p2
            ...
            · have hp1y := areCoprime_p1 x y hxy
              ...
              · have hp2y := areCoprime_p2 x y hxy
                ...
                have hy'_no_p1 : ¬(∃ yr : PreTDMonoid, mk yr = mk y' ∧ yr.exp_p1 ≥ 1) :=
                  fun h => hp1_not_y ((atomP1_dvd_iff (mk y')).mpr h)
                have hy'_no_p2 : ¬(∃ yr : PreTDMonoid, mk yr = mk y' ∧ yr.exp_p2 ≥ 1) :=
                ...
                simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2] at htot_gi
            ...
            · by_cases hp2x : atomP2 ∣ x
              ...
              · -- x has neither p1 nor p2
                rw [← hx'] at hp1x hp2x
                have hx'_no_p1 : ¬(∃ xr : PreTDMonoid, mk xr = mk x' ∧ xr.exp_p1 ≥ 1) :=
                  fun h => hp1x ((atomP1_dvd_iff (mk x')).mpr h)
                have hx'_no_p2 : ¬(∃ xr : PreTDMonoid, mk xr = mk x' ∧ xr.exp_p2 ≥ 1) :=
                ...
                simp only [PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2] at htot_fi
      ...
      · -- Surjectivity
        ...
        -- Construct preimage
        let fv : Fin 2 → TDFailMonoid := ![f₀, x₁]
        let gv : Fin 2 → TDFailMonoid := ![g₀, y₁]
...
theorem tdfail_CPL : CPL TDFailMonoid := by
  intro r
  ...
  · rw [List.pairwise_iff_get]
    ...
    · -- p = atomP1
      ...
      have hequiv := Quotient.exact hc'
      obtain ⟨hothers, htotal, _⟩ := hequiv
      simp only [PreTDMonoid.pn_others, PreTDMonoid.pn_exp_p1, PreTDMonoid.pn_exp_p2] at hothers htotal
      omega
    · -- p = atomP2
    ...
    · -- p = atomPn m
      ...
      simp only [PreTDMonoid.pn_others] at hothers1 hothers2
...
/-- Total degree of a representative; well defined on the quotient since the
    defining relation preserves `others` and the total p₁+p₂ exponent. -/
def totalDegree : TDFailMonoid → ℕ :=
  Quotient.lift
    (fun x : PreTDMonoid => x.exp_p1 + x.exp_p2 + x.others.sum (fun _ n => n))
    (by
      rintro x y ⟨ho, ht, -⟩
      simp only [ho]
      omega)

lemma totalDegree_mk (x : PreTDMonoid) :
    totalDegree (mk x) = x.exp_p1 + x.exp_p2 + x.others.sum (fun _ n => n) := rfl

lemma totalDegree_mul (x y : TDFailMonoid) :
    totalDegree (x * y) = totalDegree x + totalDegree y := by
  refine Quotient.inductionOn₂ x y fun a b => ?_
  show totalDegree (mk a * mk b) = totalDegree (mk a) + totalDegree (mk b)
  rw [mk_mul]
  simp only [totalDegree_mk, PreTDMonoid.mul_exp_p1, PreTDMonoid.mul_exp_p2,
    PreTDMonoid.mul_others]
  rw [Finsupp.sum_add_index' (fun _ => rfl) (fun _ _ _ => rfl)]
  omega

lemma totalDegree_eq_zero_iff (x : TDFailMonoid) : totalDegree x = 0 ↔ x = 1 := by
  refine Quotient.inductionOn x fun a => ?_
  constructor
  · intro h
    have h' : a.exp_p1 + a.exp_p2 + a.others.sum (fun _ n => n) = 0 := h
    have h1 : a.exp_p1 = 0 := by omega
    have h2 : a.exp_p2 = 0 := by omega
    have h3 : a.others.sum (fun _ n => n) = 0 := by omega
    have h4 : a.others = 0 := by
      rw [Finsupp.sum] at h3
      have hforall := Finset.sum_eq_zero_iff.mp h3
      ext i
      simp only [Finsupp.coe_zero, Pi.zero_apply]
      by_cases hi : i ∈ a.others.support
      · exact hforall i hi
      · exact Finsupp.notMem_support_iff.mp hi
    exact Quotient.sound ⟨by simp [h4], by simp [h1, h2], by simp [h1]⟩
  · intro h
    rw [h]
    rfl
...
theorem tdfail_WFD : WFD TDFailMonoid := by
  have hsub : Subrelation (fun a b : TDFailMonoid => StrictDvd a b)
      (InvImage (· < ·) totalDegree) := by
    intro a b hab
    obtain ⟨c, hc_unit, rfl⟩ := hab
    have hc0 : totalDegree c ≠ 0 := fun h0 =>
      hc_unit ((isUnit_iff c).mpr ((totalDegree_eq_zero_iff c).mp h0))
    show totalDegree a < totalDegree (a * c)
    rw [totalDegree_mul]
    omega
...
/-- The atom family atomPn is injective. -/
lemma atomPn_injective : Function.Injective atomPn := by
  intro m n h
  have hequiv := Quotient.exact h
  obtain ⟨hothers, -, -⟩ := hequiv
  simp only [PreTDMonoid.pn_others] at hothers
  exact (Finsupp.single_left_inj one_ne_zero).mp hothers

instance : Countable PreTDMonoid := by
  have hinj : Function.Injective
      (fun x : PreTDMonoid => (x.exp_p1, x.exp_p2, x.others)) := by
    intro a b h
    cases a; cases b
...
instance : Countable TDFailMonoid := Quotient.countable

/-- CPL⁺ holds: the atom set is countably infinite, so an enumeration of the
    atoms is a countable coprime basis. -/
theorem tdfail_CCA : CCA TDFailMonoid :=
  CCA_of_atoms_countably_infinite tdfail_reduced (Set.to_countable _)
    (Set.infinite_of_injective_forall_mem atomPn_injective
      (fun m => (atomPn_irreducible m : atomPn m ∈ Atoms TDFailMonoid)))
