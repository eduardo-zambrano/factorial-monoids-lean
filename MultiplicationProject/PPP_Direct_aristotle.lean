...
This project request had uuid: 56d09817-ceb4-411c-ba68-0ba84aa7d4c7
...
lemma atom_dividing_coprime_product_eq {M : Type*} [CommMonoid M]
  ...
  have hq_pow : q ∣ p ^ e := by
    exact h_prod ▸ dvd_mul_of_dvd_left hq_dvd _;
  exact atom_dvd_power_eq h_reduced ( Prop_CFI_implies_PPP h_reduced h_atomic h_cfi ) hp hq hq_pow
...
theorem Prop_CFI_implies_PPP_Full {M : Type*} [CommMonoid M]
  ...
  -- Apply the result that CFI implies PP-P.
  apply Prop_CFI_implies_PPP h_reduced h_atomic h_cfi;
  · exact hp;
  · -- Since $p^e = x * y$, we have $x * y \in \langle p \rangle$ by definition of powers.
    use e
